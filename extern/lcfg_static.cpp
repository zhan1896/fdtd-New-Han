   ...
   inline static void lcfg_string_grow(struct lcfg_string *s, unsigned int new_size)
{
   ...
   struct lcfg_string *lcfg_string_new()
{
	struct lcfg_string *s = (struct lcfg_string*) malloc(sizeof(struct lcfg_string));
	assert(s);
...
{
	struct lcfg_string *s_new = (struct lcfg_string *)malloc(sizeof(struct lcfg_string));
	assert(s_new);

	s_new->capacity = s->capacity;
	s_new->size = s->size;
	s_new->str = (char*) malloc(s_new->capacity);
		...
		switch( state )
		{
			case start:
				switch( c )
				{
					...
					case '\n':
						break;
					case ';': //********** CUSTOM Andrew O'Brien Custom **********/
						s->prepared_token.type = lcfg_semicolon;
						break;
					case '=':
...
{
	struct lcfg_scanner *s = (struct lcfg_scanner *)malloc(sizeof(struct lcfg_scanner));
	assert(s);
   ...
   static int lcfg_parser_add_value(struct lcfg_parser *p, const char *key, struct lcfg_string *value)
{
	...
	{
		p->value_capacity *= 2;
		p->values = (struct lcfg_parser_value_pair *) realloc(p->values, sizeof(struct lcfg_parser_value_pair) * p->value_capacity);
		assert(p->values);
	}
...
{
	struct lcfg_parser *p = (struct lcfg_parser *) malloc(sizeof(struct lcfg_parser));
	assert(p);
	...
	p->value_length = 0;
	p->value_capacity = 8;
	p->values = (struct lcfg_parser_value_pair *) malloc(sizeof(struct lcfg_parser_value_pair) * p->value_capacity);
	assert(p->values);
...
#define STATE_STACK_PUSH(t) \
	...
	{ \
		state_stack_size *= 2; \
		state_stack = (struct state_element *) realloc(state_stack, state_stack_size * sizeof(struct state_element)); \
	} \
	state_stack[++ssi].s = t; \
...
#define PATH_POP() \
	...
	struct state_element *state_stack = (struct state_element *) malloc(sizeof(struct state_element) * state_stack_size);
	...
	while( lcfg_scanner_has_next(p->scanner) && state_stack[ssi].s != invalid )
	{
		...
		switch( state_stack[ssi].s )
		{
			...
			case exp_equals:
                ...
                //********** START Andrew O'Brien Custom **********/
				else if( t.type == lcfg_semicolon )
                {   
                    struct lcfg_string *func;
                    func = lcfg_string_new();
                    lcfg_string_cat_cstr(func, "__execute");
                    
                    lcfg_parser_add_value(p, lcfg_string_cstr(current_path), func);
...
{
	struct lcfg *c = (struct lcfg *) malloc(sizeof(struct lcfg));
	assert(c);
...
{
	struct lcfgx_tree_node *node = (struct lcfgx_tree_node *) malloc(sizeof(struct lcfgx_tree_node));
...
{
	struct lcfgx_tree_node *root = (struct lcfgx_tree_node *) user_data;
